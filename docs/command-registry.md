...
`:describe`/`:desc [table]`, `:stats [column]`, `:bar [label] [value] [sum|count|avg]`, `:line [x] [y]`, `:format`, `:theme <name>`.
...
- `:explain`→`explainQuery`, `:stats`→`fetchColumnStats`, `:bar`→`exBar`, `:line`→`exLine`,
