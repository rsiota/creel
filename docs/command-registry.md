...
`:describe`/`:desc [table]`, `:stats [column]`, `:bar[!] [label] [value] [sum|count|avg]`, `:line[!] [x] [y]`, `:hist[!] [column] [bins]`, `:format`, `:theme <name>`.
...
- `:explain`→`explainQuery`, `:stats`→`fetchColumnStats`, `:bar`→`exBar`, `:line`→`exLine`, `:hist`→`exHist`,
