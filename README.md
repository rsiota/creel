...
- **Static ERD** (`g R` / `:erd`) — renders a graphical entity-relationship diagram in a scrollable panel: bordered table cards laid out in dependency-ranked columns with box-drawing arrows from each FK to the PK it references (◆ = PK, ◇ = FK). `m` toggles to the Mermaid `erDiagram` source; `y`/`s` copy/save the Mermaid (renders natively in GitHub/GitLab markdown), `h/l` pan wide schemas. Click a card to highlight its relationships (dimming the rest), double-click to re-centre on it; in a focused ERD (`:erd <table>`) each related table's header is clickable (marked with a `⤢`) to re-focus the diagram on that table's neighbourhood and walk the graph without leaving the panel
