...
- **Static ERD** (`g R` / `:erd`) — generates a Mermaid `erDiagram` of the schema (or a table's FK neighbourhood) shown in a scrollable panel: entities with `PK`/`FK` columns and one-to-many relationship edges. Copy to clipboard (`y`), save to a `.mmd` file (`s` / `:erd save [file]`) — renders natively in GitHub/GitLab markdown
...
| `g R`      | Static ERD (Mermaid diagram)                 |
