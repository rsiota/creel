...
func TestBuildCreateDatabaseSQL(t *testing.T) {
	tests := []struct {
		...
		{DriverMySQL, "mydb", "CREATE DATABASE `mydb`", false},
		{DriverMySQL, "new_db_2", "CREATE DATABASE `new_db_2`", false},
		{DriverMySQL, "", "", true},
		{DriverMySQL, "  ", "", true},
		{DriverMySQL, "inj';ection", "", true},
		{DriverSQLite, "mydb", "", true},
	}
	for _, tc := range tests {
		got, err := BuildCreateDatabaseSQL(tc.driver, tc.name)
		if tc.err {
			if err == nil {
				t.Errorf("BuildCreateDatabaseSQL(%q, %q) expected error, got %q", tc.driver, tc.name, got)
			}
			continue
		}
		if err != nil {
			t.Errorf("BuildCreateDatabaseSQL(%q, %q) unexpected error: %v", tc.driver, tc.name, err)
			continue
		}
		if got != tc.want {
			t.Errorf("BuildCreateDatabaseSQL(%q, %q) = %q, want %q", tc.driver, tc.name, got, tc.want)
		}
	}
}
